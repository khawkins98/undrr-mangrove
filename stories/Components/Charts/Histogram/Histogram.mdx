...
* **`dataSeries`**: `Array<{ seriesLabel: string, data: number[], color: string }>` - An array of objects, each containing a label, a dataset, and a color. *(required)*
* **`width`**: `number` - The width of the chart in pixels. *(required)*
* **`height`**: `number` - The height of the chart in pixels. *(required)*
* **`bins`**: `number` - The number of bins to divide the data into. *(default: 30)*
* **`locale`**: `string` - The locale for the chart, e.g., `"en"` or `"ar"`. *(default: `"en"`)*
* **`direction`**: `string` - The text direction, `"ltr"` or `"rtl"`. *(default: `"ltr"`)*
